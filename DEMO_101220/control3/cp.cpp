...
io_write (resmgr_context_t *ctp, io_write_t *msg, iofunc_ocb_t *ocb)
{
    ...
    sts = iofunc_write_verify (ctp, msg, ocb, NULL); //checks if we have the right to write
    if ( sts != EOK ){
        return (sts);
    }
    ...
    //Checks if the buffer/message is weird, if so throw it away?
    if(resmgr_msgread(ctp, buffer, nbytes, doffset) == -1){
        free(buffer);
        return (errno);
    }
    
    process_data (off, buffer, nbytes); //Checks the msg and chooses what process to do next [switch-case]?
