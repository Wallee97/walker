...
double f_b, t_b;                         /* Àiu[LÍCñ]pu[LÍ */

double H;                               /* ã\_ÜÅÌ£ */
double DH;                              /* fpÌã\_ÜÅÌ£*/

/* Parameter for Avoidance Control */

double F_R={0.};                         /* Repulsive Force ()ËÍ          */  
double TH_R;                            /* Repulsive Force@Ìûü           */
double P_q;                              /* áQšÆwalkerÌ§ä_ÆÌÅ¬£*/
double Cross_p[2];                       /* Walker©ç©œáQšÉêÔß¢_  */  
...
LMSAvoidCtrl(struct params *param, int trig)
{
	...
	if(ticks%50==0){
	  ...
	  if((Cur_e.velocity < 0.03) && (Cur_e.velocity > -0.03) && (fh_dis > 70.) && (fh_dis < 200.) && (mindata_d[yHip] > mindata_d[YG])){
	    ...
	    //³KªzÉµœª€]|h~@\
	    //2³Ì³KªzÉæèèlðèµAèlð±Šœçu[Lª©©é
...
/* getObstacleAbsoluteCoordinate                                  */
/* URG©çŸçêœ«ÌÅ¬£Ì_ÌlÌxAyðàÆßé   @@@   */
...
/* 	y = 1.0 & y = -1.0 ðÇÆµœáQšñð[h                   */
/*  @€m[gS@QÆ @@@@@@@@@@@@@@@@@@@@ */
...
/* ÎßŒüðáQšÆµœáQšñð[h @@@                    */
/* €m[gS@QÆ                                             */
/*                                                                  */
void
...
/* ¿_(2.0, 1.5)ðáQšÆµœáQšñð[h                    */
/* €m[gS@QÆ@@@@@@@@@@@@@@@@@@@@@@*/
/*                                                                 */
void
...
/* RepulsiveForce                                                  */
/* âÎÀWËÍ¬ªCËÍCËÍûü@ðvZ                    */
void
RepulsiveForce(void)
{
...
/* getNearestDistance                                               */
/* Walker©çáQšÖÌÅßÚ£ P_q ðßé                      */
void
...
/*iÚWCWj[iÀÛÌCWjÚWÉµÄ¢é@@@@@@@*/
/* ž[ÁÆ±êê{ÅâÁÄ¢Üµœ@@@@@@@@@@@@@@@@@*/
void
ModelCtrlOnly(struct params *param, int trig)
{
  ...
  if( Cur_e.velocity > 0 ){
    /*isûüu[LÍ*/
        f_b = des_Cur_e.dmp[V]*Cur_e.velocity ;
    /*ñ]ûüu[LÍ*/
        /* t_b = - des_Cur_e.dmp[TH]*Cur_e.omega ; */          /*Ž³ño[W*/
		t_b = - des_Cur_e.dmp[TH]*Cur_e.omega*H*H ;            /*  LX^[Á« */
  } 
  else{
        f_b = -des_Cur_e.dmp[V]*Cur_e.velocity ;
        /* t_b = + des_Cur_e.dmp[TH]*Cur_e.omega ; */          /*Ž³ño[W*/
...
/*ââ±µ¢Å·ªC±êÍÚWÌCD_dðŒÚwèµÄ¢é^®Á«ÂÏ§ävO*/
/*AµAŒiûüÌÝDñ]ÉÖµÄÍ·ðR}hüÍµÄ­Ÿ³¢                   */
void
ModelCtrlOnly2(struct params *param, int trig)
{
	static double Dam = 10.; /*ÀÛÌisûüÉÖ·éCWðüÍ*/
	double dam;
...
TraCtrl2(struct params *param, int trig)
{
 ...
 /*@oHÆ»ÝnÆÌÅ¬£@*/
  ...
  /*des_th = atan(Cur_e.pos[X]/(Cur_e.pos[Y]-R)); *//*@ÚWpx(Úüpx)@*/
